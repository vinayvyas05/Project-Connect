...
function DeleteConfirmModal({ teamName, onConfirm, onCancel, loading }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-surface border border-gray-200 rounded-2xl p-6 w-full max-w-sm mx-4 shadow-2xl animate-page-in">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-text-primary font-semibold text-sm">Delete team</h3>
            <p className="text-text-secondary text-xs">This action cannot be undone</p>
          </div>
        </div>

        <p className="text-text-secondary text-sm mb-6">
          Are you sure you want to delete <strong className="text-text-primary">{teamName}</strong>?
        ...
        <div className="flex items-center gap-3 justify-end">
          <button
            onClick={onCancel}
            disabled={loading}
            className="px-4 py-2 rounded-lg text-sm font-medium text-text-secondary bg-gray-100 hover:bg-gray-200 transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            disabled={loading}
            className="px-4 py-2 rounded-lg text-sm font-medium text-white bg-error hover:bg-error/90 transition-colors disabled:opacity-50 shadow-sm"
          >
...
export default function ChannelList({
  ...
  return (
    <aside className="w-64 flex flex-col shrink-0 bg-sidebar border-r border-gray-200 relative z-20 overflow-hidden text-text-primary h-full">
      {/* Team name header */}
      <div className="px-5 py-5 border-b border-gray-200 bg-white/50">
        {renamingTeam ? (
          <div className="animate-fade-in">
            <input
              ...
              className="w-full bg-white border border-primary/50 rounded-lg px-3 py-2 text-sm text-text-primary placeholder-gray-400 outline-none focus:ring-2 focus:ring-primary/20 disabled:opacity-50 transition-all font-medium shadow-sm"
            />
            {teamRenameError && (
              <p className="text-[10px] text-error mt-1.5 px-1 font-medium italic">
                {teamRenameError}
              </p>
            )}
            <p className="text-[10px] text-text-secondary mt-1.5 px-1 uppercase tracking-tighter opacity-70">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col min-w-0">
              <h2 className="font-bold text-text-primary truncate text-base tracking-tight">
                {team ? team.name : "Select a team"}
              </h2>
              <div className="flex items-center gap-1.5 mt-0.5">
                <div className="w-1.5 h-1.5 rounded-full bg-success animate-pulse shadow-sm" />
            ...
            {isOwner && (
              <div className="flex items-center gap-2 pt-3 border-t border-gray-200">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 px-2 py-2 bg-white hover:bg-gray-50 text-text-secondary hover:text-text-primary rounded-xl transition-all text-[11px] font-bold border border-gray-200 shadow-sm active:scale-95 group"
                >
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 px-2 py-2 bg-error/5 hover:bg-error/10 text-error/80 hover:text-error rounded-xl transition-all text-[11px] font-bold border border-error/20 shadow-sm active:scale-95 group"
                >
                  <svg className="w-3.5 h-3.5 opacity-60 group-hover:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5}
        ...
        {team ? (
          channelsLoading ? (
            ...
            <div className="px-2 space-y-6">
              <div>
                <div className="h-2 w-16 bg-gray-200 rounded-full animate-pulse mb-4" />
                <div className="space-y-3">
                  {[75, 55, 85, 65].map((w, i) => (
                    <div key={i} className="flex items-center gap-3">
                      <div className="w-2.5 h-2.5 bg-gray-200 rounded-full animate-pulse shrink-0" />
                      <div className="h-2.5 bg-gray-200 rounded-full animate-pulse" style={{ width: `${w}%` }} />
                    </div>
                  ))}
                </div>
              </div>
            ...
            <>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between px-3 mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-gray-100 text-text-secondary border border-gray-200">
                  ...
                  <button
                    ...
                    className={`p-1 rounded-lg transition-all ${
                      atLimit
                        ...
                        : "text-text-secondary hover:text-text-primary hover:bg-gray-100 border border-transparent hover:border-gray-200"
                    }`}
                  >
                ...
                <div className="space-y-0.5">
                  {channels.length === 0 ? (
                    <p className="text-text-secondary text-[11px] font-medium px-3 italic">
                      No channels yet...
                    </p>
                  ) : (
                    channels.map((ch) => (
                        ...
                        {renamingId === ch._id ? (
                          <div className="px-2 py-2">
                            <input
                              ...
                              className="w-full bg-white border border-primary/50 rounded-lg px-2 py-1.5 text-xs text-text-primary outline-none font-medium shadow-sm"
                            />
                          </div>
                        ) : (
                          <button
                            ...
                            ${
                              activeChannelId === ch._id
                                ...
                                : "text-text-secondary hover:text-text-primary border-transparent hover:bg-white hover:border-gray-200 hover:shadow-sm"
                            }`}
                          >
                            <span className={`transition-colors duration-200 ${
                              activeChannelId === ch._id ? "text-primary" : "text-gray-400 group-hover:text-text-secondary"
                            }`}>#</span>
                            ...
                            {isAdmin && (
                              <span
                                ...
                                className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-lg hover:text-text-primary text-text-secondary shrink-0 hover:bg-gray-100"
                              >
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="space-y-0.5">
                  ...
                  >
                    <svg className={`w-4 h-4 shrink-0 transition-colors ${membersActive ? 'text-primary' : 'text-text-secondary'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  >
                    <svg className={`w-4 h-4 shrink-0 transition-colors ${location.pathname === `/teams/${team._id}/tasks` ? 'text-primary' : 'text-text-secondary'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="flex flex-col items-center justify-center h-40 text-center px-4">
             <div className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center mb-4 border border-gray-200 shadow-sm">
      ...
      <div className="mt-auto p-4 bg-sidebar border-t border-gray-200">
        <UserFooter />
      </div>
